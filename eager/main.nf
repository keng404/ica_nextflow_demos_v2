...
process hostremoval_input_fastq {
    ...
    if ( seqtype == 'SE' ) {
        ...
        """
        samtools index $bam
    python ${workflow.launchDir}/bin/extract_map_reads.py $bam ${r1} -m ${params.hostremoval_mode} $merged -of $out_fwd -t ${task.cpus}
        """
    } else {
        ...
        """
        samtools index $bam
    python ${workflow.launchDir}/bin/extract_map_reads.py $bam ${r1} -rev ${r2} -m ${params.hostremoval_mode} $merged -of $out_fwd -or $out_rev -t ${task.cpus}
        """ 
    }
...
process samtools_filter {
    ...
    python ${workflow.launchDir}/bin/filter_bam_fragment_length.py -a -l ${params.bam_filter_minreadlength} -o ${libraryid} tmp_mapped.bam
        samtools index ${libraryid}.filtered.bam ${size}
        """
    } else if ( "${params.bam_unmapped_type}" == "discard" && params.bam_filter_minreadlength != 0 ){
        """
...
// Add a file entry to match expected no. tuple elements for     python ${workflow.launchDir}/bin/endorS.py even if not giving second file
  ch_flagstat_for_endorspy
    .map { it -> 
        def samplename = it[0]
        def libraryid  = it[1]
...
process endorSpy {
    ...
    if (params.run_bam_filtering) {
      """
    python ${workflow.launchDir}/bin/endorS.py -o json -n ${libraryid} ${stats} ${poststats}
      """
    } else {
      """
    python ${workflow.launchDir}/bin/endorS.py -o json -n ${libraryid} ${stats}
      """
    }
}
...
process eigenstrat_snp_coverage {
  ...
  */
  ...
  eigenstrat_snp_coverage -i pileupcaller.${strandedness} >${strandedness}_eigenstrat_coverage.txt
    python ${workflow.launchDir}/bin/parse_snp_cov.py ${strandedness}_eigenstrat_coverage.txt
  """
}
...
process print_nuclear_contamination{
    ...
    script:
    """
    python ${workflow.launchDir}/bin/print_x_contamination.py ${Contam.join( )}
    """
 }
/////////////////////////////////////////////////////////
...
process kraken_parse {
  ...
  """
    python ${workflow.launchDir}/bin/kraken_parse.py -c ${params.metagenomic_min_support_reads} -or $read_out -ok $kmer_out $kraken_r
  """    
}
process kraken_merge {
  ...
  """
    python ${workflow.launchDir}/bin/merge_kraken_res.py -or $read_out -ok $kmer_out
  """    
}
//////////////////////////////////////
/* --    PIPELINE COMPLETION     -- */
...
process get_software_versions {
  label 'mc_small'
    ...
    python ${workflow.launchDir}/bin/endorS.py --version &> v_endorSpy.txt || true

...
process library_merging_and_alignment {
    ...
    script:
    """
    python ${workflow.launchDir}/bin/merge_and_align_libraries_from_easypqp.py \\
...
process pseudo_irt_generation {
    ...
    script:
    """
    python ${workflow.launchDir}/bin/select_pseudo_irts_from_lib.py \\
...
process chromatogram_alignment {
    ...
    Rscript ${workflow.launchDir}/bin/DIAlignR.R \\
...
process reformatting {
    ...
    python ${workflow.launchDir}/bin/reformat_output_for_msstats.py \\
        --input ${dialignr_file} \\
...
process mztab_export {
    ...
    """
    TargetedFileConverter -in ${lib_file} -out ${lib_file.baseName}.tsv
    python ${workflow.launchDir}/bin/mztab_output.py \\
        --input ${dialignr_file} \\
        --exp_design ${exp_design} \\
        --library ${lib_file.baseName}.tsv \\
        --fdr_level ${params.pyprophet_global_fdr_level} \\
...
process statistical_post_processing {
    ...
    script:
    """
    Rscript ${workflow.launchDir}/bin/msstats.R > msstats.log || echo "Optional MSstats step failed. Please check logs and re-run or do a manual statistical analysis."
    """
}
/*
...
process output_visualization {
    ...
    script:
    """
    Rscript ${workflow.launchDir}/bin/plot_quantities_and_counts.R ${sample}
    """
}
/*
